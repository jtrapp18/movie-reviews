...
def validate_required_string(value: str, field_name: str, min_length: int = 1, max_length: Optional[int] = None, to_lower: bool = False, strip: bool = True) -> str:
...
def validate_date_or_yyyy_mm_dd(value, field_name: str):
    ...
    if isinstance(value, str):
        try:
            ...
            raise ValueError(f"Invalid date format for {field_name}. Must be YYYY-MM-DD.")
    raise ValueError(f"Invalid date format for {field_name}. Must be a string or date object.")
...
def validate_optional_int_in_range(value, min_value: int, max_value: int, field_name: str):
    if value is None:
        return None
    if not isinstance(value, int) or value < min_value or value > max_value:
        raise ValueError(f"{field_name} must be an integer between {min_value} and {max_value}, or None.")
    return value


def normalize_tag_name(value: str, max_length: int = 50) -> str:
    result = validate_required_string(value, 'Tag name', min_length=1, max_length=max_length, to_lower=True)
    return result


def validate_optional_email(value: Optional[str], field_name: str = 'Email') -> Optional[str]:
...
def validate_optional_phone(value: Optional[str], field_name: str = 'Phone number') -> Optional[str]:
    if not value:
        return value
