...
class DocumentProcessor:
    ...
    def allowed_file(filename: str) -> bool:
        ...
        return (
            ...
            in DocumentProcessor.ALLOWED_EXTENSIONS
        )
        ...
        if remove_title:
            ...
            if len(lines) > 3:  # Only process if we have enough content
                ...
                for i, line in enumerate(lines[:3]):
                    ...
                    is_title = any(
                        re.match(pattern, line) for pattern in title_patterns
                    )
                    ...
                    if is_title:
                        ...
                        if next_content_line and (
                            len(next_content_line) > len(line) * 2
                            or any(
                                word in next_content_line.lower()
                                for word in [
                                    "the",
                                    "and",
                                    "of",
    ...
    def convert_text_to_html(text: str) -> str:
        ...
        for line in lines:
            ...
            is_header = len(line) < 100 and (
                line.isupper()  # ALL CAPS
                or re.match(r"^[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*$", line)  # Title Case
                or re.match(r"^[A-Z][A-Z\s]+$", line)  # Mixed caps
            )
    ...
    def extract_text_from_document(
        ...
        if clean_text and raw_text:
            cleaned_text = DocumentProcessor.clean_extracted_text(
                raw_text, remove_title
            )
        else:
    ...
    @staticmethod
    def extract_html_from_document(
        file_path: str,
        file_type: str,
        clean_text: bool = True,
        remove_title: bool = True,
                ...
                for page_num, page in enumerate(pdf.pages):
                    ...
                    for img_num, img in enumerate(page_images):
                        try:
                            ...
                            print(
                                f"Error extracting image {img_num} from page {page_num}: {e}"
                            )
                            continue
        except Exception as e:
    ...
    def get_document_preview(
        file_path: str, file_type: str, max_chars: int = 500
    ) -> str:
        """Get a preview of the document content."""
        try:
            full_text = DocumentProcessor.extract_text_from_document(
                file_path, file_type, clean_text=True, remove_title=True
            )
